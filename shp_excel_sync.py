...
def get_fk_set(layerName, fkName, skipFirst=1, fids=None, useProvider=False):
    ...
    for f in feats[skipFirst:]:
        QgsMessageLog.logMessage('FK {}'.format(f.attribute(fkName)), logTag, QgsMessageLog.CRITICAL)
...
def removed_geom_precommit(fids):
    ...
    fks_to_remove = get_fk_set(shpName,shpKeyName,skipFirst=0,fids=fids,useProvider=True)
    global shpRemove
...
def updateShpLayer(fksToRemove):
    ...
    prompt_msg = "Attempt to synchronize between Excel and Shapefile. Shapefile has features with ids: ({}) that don't appear in the Excel. Delete those features from the shapefile? ".format(','.join([str(fk) for fk in fksToRemove]))
